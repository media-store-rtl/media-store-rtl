...
class LinkJob implements ShouldQueue
{
    ...
    public function handle(User $user , Role $role)
    {
        ...
        if($links->status == 0)
        {
            ...
            $message = 'لینک شبکه اجتماعی شما در وضیعت انتظار قرار گرفت.';
            $route = 'social.index';
            $user = $user->find($links->user_id);
            Notification::send($user , new LinkNotification($links,$message,$route,$user));

            $message = 'یک لینک شبکه اجتماعی نیاز به تایید دارد.';
        ...
        {
            $message = 'لینک شما توسط کارشناس تایید و در صفحه شبکه های اجتماعی نمایش داده شد.';
            $route = 'social.index';
