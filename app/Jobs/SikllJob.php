...
class SikllJob implements ShouldQueue
{
    ...
    public function handle(User $user , Role $role)
    {
        ...
        if($siklls->status == 0)
        {
            ...
            $message = 'مهارت شما در وضیعت انتظار قرار گرفت.';
            $route = 'sikll.index';
            $user = $user->find($siklls->user_id);
            Notification::send($user , new SikllNotification($siklls,$message,$route,$user));

            $message = 'یک مهارت نیاز به تایید دارد.';
        ...
        {
            $message = 'مهارت شما توسط کارشناس تایید و در صفحه مهارت نمایش داده شد.';
            $route = 'sikll.index';
