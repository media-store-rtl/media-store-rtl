...
class IdentityJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        if ($identitys->status == 0) {
            ...
            $message = 'اطلاعات هویتی شما در وضعیت انتظار قرار گرفت.';
            ...
            $message = 'اطلاعات هویتی شخصی نیاز به تایید دارد.';
            $route = 'identityAdmin.index';

            $users = Role::find(3); // فرض بر این است که نقش 3، نقش ادمین است
            foreach ($users->users as $key => $user)
            {
                Notification::send($user , new IdentityNotification($identitys,$message,$route,$user));
            }

        } elseif ($identitys->status == 4) {
