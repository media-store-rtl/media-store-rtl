...
class ProductJob implements ShouldQueue
{
    ...
    public function handle(User $user , Role $role )
    {
        ...
        if($products->status == 0)
        {
            ...
            $message = 'محصول شما در وضیعت انتظار قرار گرفت.';
            $route = 'product.index';
            $user = $user->find($products->user_id);
            Notification::send($user , new ProductNotification($products,$message,$route,$user));

            $message = 'یک محصول نیاز به تایید دارد.';
        ...
        {
            $message = 'محصول شما توسط کارشناس تایید و در صفحه محصولات نمایش داده شد.';
            $route = 'product.index';
        ...
        else if($products->status == 5)
        {
            ...
            $message = 'محصول شما در وضیعت توقف فروش قرار گرفت.';
            $route = 'product.index';
            $user = $user->find($products->user_id);
            Notification::send($user , new ProductNotification($products,$message,$route,$user));
