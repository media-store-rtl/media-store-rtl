...
const _sfc_main = {
  ...
  props: {
    ...
    ids: { type: [Object, String], default: () => ({}) },
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (props.orders.total > 0) {
        ...
        if (props.orders) {
          ...
          if (props.orders.total > 0) {
            _push(`<table class="table table-responsive"><thead><tr class="col"><th scope="col">شناسه</th><th scope="col">قیمت</th><th scope="col">تعداد</th><th scope="col">تخفیف</th><th scope="col">جمع</th><th scope="col">کمیسیون</th><th scope="col">مالیات</th><th scope="col">در آمد</th><th scope="col">تاریخ</th></tr></thead><tbody><!--[-->`);
            ssrRenderList(props.orders.data, (order, index) => {
              _push(`<tr><td>${ssrInterpolate(order.id.toLocaleString("fa-IR"))}</td><td>${ssrInterpolate(order.price.toLocaleString("fa-IR"))}</td><td>${ssrInterpolate(order.count.toLocaleString("fa-IR"))}</td><td>${ssrInterpolate(order.discount.toLocaleString("fa-IR"))}</td><td>${ssrInterpolate(order.total.toLocaleString("fa-IR"))}</td><td>${ssrInterpolate(order.comison.toLocaleString("fa-IR"))}</td><td>${ssrInterpolate(order.tax.toLocaleString("fa-IR"))}</td><td>${ssrInterpolate(order.col.toLocaleString("fa-IR"))}</td><td>${ssrInterpolate(unref(moment)(order.created_at).locale("fa", unref(fa)).format("jYYYY/jM/jD HH:mm"))}</td></tr>`);
            });
            _push(`<!--]--></tbody></table>`);
          } else {
            _push(`<!---->`);
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Users/Seller/Product/product-show.vue");
