...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`</select></div><div class="col-3 mb-3"><label class="form-label">حساب بانک<span class="text-danger">*</span></label><select class="form-select" name="bank" id="" autocomplete="bank">`);
      if (banks.value.length > 0) {
        _push(`<!--[-->`);
        ssrRenderList(banks.value, (bank, index) => {
          _push(`<option${ssrRenderAttr("value", bank)}${ssrIncludeBooleanAttr(Array.isArray(unref(form).bank) ? ssrLooseContain(unref(form).bank, bank) : ssrLooseEqual(unref(form).bank, bank)) ? " selected" : ""}>${ssrInterpolate(bank.menu.name)}-${ssrInterpolate(bank.shaba_number)}</option>`);
        });
        _push(`<!--]-->`);
      } else {
      ...
      _push(`</select></div><div class="col-lg-3 mb-3"><label class="form-label">تاریخ<span class="text-danger">*</span></label>`);
      ...
      _push(`</div><div class="col-lg-3 mb-3"><label class="form-label">مبلغ<span class="text-danger">*</span></label><input type="text"${ssrRenderAttr("value", unref(form).price)} class="form-control"></div></div></div></div><br></form></section></div></div></div></div></section>`);
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Users/Buyer/Mali/payment-create.vue");
