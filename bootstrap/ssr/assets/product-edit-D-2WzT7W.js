      ...
      if (step.value == "step3") {
        ...
        if (unref(form).test) {
          ...
          ssrRenderList(unref(form).test, (tes, index) => {
            _push(`<div class="col-lg-6"><label class="form-label">نسخه تست شده محصول (${ssrInterpolate(tes.name)})</label>`);
            _push(ssrRenderComponent(unref(VueMultiselect), {
        ...
        _push(`</div><div class="row gx-2"><label class="form-label">تگ توضیحات سئو: طول کارکتر بین ۱۲۰ تا ۱۶۰ کاراکتر و شامل کلمه کلیدی اصلی</label><textarea class="form-control w-100" name="comment" id="comment" cols="30" rows="9" placeholder="اینجا تایپ کنید">${ssrInterpolate(unref(form).tag)}</textarea></div></div></div></div>`);
      } else {
        _push(`<!---->`);
      }
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Users/Seller/Product/product-edit.vue");
