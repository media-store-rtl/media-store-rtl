import { computed, unref, useSSRContext } from "vue";
import { ssrRenderList, ssrRenderStyle, ssrRenderAttr, ssrInterpolate, ssrRenderClass, ssrIncludeBooleanAttr } from "vue/server-renderer";
import { a as usePage, u as useForm } from "../ssr.js";
import "@inertiajs/core";
import "es-toolkit";
import "es-toolkit/compat";
import "@inertiajs/core/server";
import "@vue/server-renderer";
import "@unhead/vue";
import "@unhead/vue/polyfill";
const _sfc_main = {
  __name: "support-reply",
  __ssrInlineRender: true,
  props: { auth: Object, users: Object, tickets: Object, replies: Object },
  setup(__props) {
    const errors = computed(() => usePage().props.errors);
    computed(() => Object.keys(errors.value).length > 0);
    const props = __props;
    const form = useForm({
      parent_id: props.tickets.id,
      menu: props.tickets.menu,
      recepiant: props.tickets.recepiant,
      subject: props.tickets.subject,
      text: null,
      destination: null,
      file: null
    });
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<!--[-->`);
      ssrRenderList(props.replies, (ticket, index) => {
        _push(`<div class="col-sm-12 mt-3"><form><div class="d-flex me-2">`);
        if (props.tickets.userimage[0] && props.tickets.userimage[0].status == 4) {
          _push(`<img style="${ssrRenderStyle({ "height": "40px" })}" class="img-xs rounded-circle"${ssrRenderAttr("src", "/../storage/" + props.tickets.userimage[0].url)} alt="User">`);
        } else {
          _push(`<img style="${ssrRenderStyle({ "height": "40px" })}" class="img-xs rounded-circle" src="/../storage/files/default-user.png" alt="User">`);
        }
        _push(`<span class="font-xs text-muted me-1">${ssrInterpolate(ticket.user.user_name)}|${ssrInterpolate(ticket.created_at)}</span></div><div class="mb-4 me-5"><p>${ssrInterpolate(ticket.text)} `);
        if (ticket.user.id !== props.auth.user.id) {
          _push(`<a${ssrRenderAttr("href", "#CollapseExample" + ticket.id)} class="reply" data-bs-toggle="collapse" aria-expanded="false"${ssrRenderAttr("aria-controls", "CollapseExample" + ticket.id)}>پاسخ</a>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</p></div><div class="submenu text-a-r collapse"${ssrRenderAttr("id", "CollapseExample" + ticket.id)}><div class="card-body"><div class="mb-4"><label class="form-label">پاسخ:</label><textarea name="text" placeholder="اینجا تایپ کنید" class="form-control" rows="4">${ssrInterpolate(unref(form).text)}</textarea></div></div><div class="card-body"><label class="form-label"> آپلودفایل<span class="text-danger me-1"></span></label><div class="input-upload"><input name="file" class="form-control" type="file" id="file">`);
        if (unref(form).progress) {
          _push(`<progress${ssrRenderAttr("value", unref(form).progress.percentage)} max="5">${ssrInterpolate(unref(form).progress.percentage)}% </progress>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div></div><button class="${ssrRenderClass([{ "opacity-25": unref(form).processing }, "btn btn-primary me-4"])}"${ssrIncludeBooleanAttr(unref(form).processing) ? " disabled" : ""}>ثبت</button></div>`);
        if (ticket.file) {
          _push(`<div class="mb-4 me-5"><p> جهت مشاهده فایل های ضمیه را دانلود نمایید.</p><a${ssrRenderAttr("href", _ctx.$page.props.ziggy.url + "/storage/" + ticket.file.url)} class="btn btn-sm btn-primary">دانلود</a></div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</form><hr></div>`);
      });
      _push(`<!--]-->`);
    };
  }
};
const _sfc_setup = _sfc_main.setup;
_sfc_main.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Users/Seller/Support/support-reply.vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};
export {
  _sfc_main as default
};
