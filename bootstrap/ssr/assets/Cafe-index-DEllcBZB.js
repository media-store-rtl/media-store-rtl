...
const titleSeo = "خدمات آنلاین اینترنت – سریع، اقتصادی و قابل اعتماد";
const descriptionSeo = "اگر دنبال راهی سریع‌تر و مقرون‌به‌صرفه برای خدمات آنلاین اینترنت هستید، استفاده از خدمات آنلاین اینترنت یک گزینه عالی هستن.ما مجموعه‌ای از خدمات آنلاین اینترنت آماده کردیم که می‌توانید و در کوتاه‌ترین زمان ممکن کار های اینترنتی خود رو به ما بسپارید.";
const _sfc_main = {
  __name: "Cafe-index",
          ...
          element.routes.forEach((route2) => {
            if (route2.name == props.path) {
              element.sections.forEach((section) => {
                if (section.name == "internet-cafes") {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (props.results && props.results.total > 0) {
        ...
        if (props.results && props.results.total > 0) {
          ...
          ssrRenderList(props.results.data, (result, index) => {
            ...
            _push(`</h2><div><span class="font-small text-muted">${ssrInterpolate(result.tag)}</span></div><div><span class="font-small text-muted">${ssrInterpolate(result.rouzekari)} روز کاری</span></div><div class="product-rate-cover"><div class="product-rate d-inline-block">`);
      ...
      if (props.orders.length > 0) {
        ...
        ssrRenderList(props.orders, (result, index) => {
          ...
          if (result.image && result.image.status == 4) {
            _push(ssrRenderComponent(unref(link_default), {
              href: _ctx.route("cafe-net.show", [result.product.slug])
            }, {
              default: withCtx((_, _push2, _parent2, _scopeId) => {
                if (_push2) {
      ...
      if (props.favorites && props.favorites.length > 0) {
        ...
        ssrRenderList(props.favorites, (result, index) => {
          _push(`<article class="row align-items-center hover-up"><figure class="col-md-4 mb-0">`);
          _push(ssrRenderComponent(unref(link_default), {
            href: _ctx.route("cafe-net.show", [result.favoritable.slug])
          }, {
            default: withCtx((_, _push2, _parent2, _scopeId) => {
              if (_push2) {
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Guest/Cafe-index.vue");
