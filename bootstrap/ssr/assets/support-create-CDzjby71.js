  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (step.value == "step2") {
        ...
        if (subs.value == true) {
          _push(`<div class="col-sm-4 me-1" id="product"><label class="form-label">کاربر<span class="text-danger me-1">*</span></label><select name="product" class="form-select" aria-label="select example"><!--[-->`);
          ssrRenderList(orders.value, (order, index) => {
            _push(`<option${ssrRenderAttr("value", order)}${ssrIncludeBooleanAttr(Array.isArray(unref(form).destination) ? ssrLooseContain(unref(form).destination, order) : ssrLooseEqual(unref(form).destination, order)) ? " selected" : ""}>`);
            if (order.name) {
              _push(`<!--[-->${ssrInterpolate(order.name_show)}<!--]-->`);
            } else {
              _push(`<!---->`);
            }
            _push(`</option>`);
          ...
          _push(`<!--]-->`);
          if (orders.value == "") {
            _push(`<option disabled${ssrIncludeBooleanAttr(Array.isArray(unref(form).destination) ? ssrLooseContain(unref(form).destination, null) : ssrLooseEqual(unref(form).destination, null)) ? " selected" : ""}>گزینه ای یافت نشد.</option>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</select></div>`);
      ...
      if (step.value == "step" || subs.value == true && unref(form).destination || subs.value == false) {
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Users/Admin/Support/support-create.vue");
