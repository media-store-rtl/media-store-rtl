    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (props.contracts.total > 0) {
        ...
        if (props.contracts) {
          ...
          if (props.contracts.total > 0) {
            _push(`<div class="table-responsive"><div class="table table-hover"><thead><tr class="col"><th scope="col">شناسه</th><th scope="col">شناسه سفارش</th><th scope="col">کارفرما</th><th scope="col">پلن</th><th scope="col">تاریخ</th><th scope="col">بروز رسانی</th><th scope="col">وضعیت</th><th scope="col">عملیات</th></tr></thead><tbody><!--[-->`);
            ssrRenderList(props.contracts.data, (tarahi, index) => {
              _push(`<tr><td>${ssrInterpolate(tarahi.id.toLocaleString("fa-IR"))}</td><td>${ssrInterpolate(tarahi.order_id)}</td>`);
              if (tarahi.user) {
                _push(`<td>`);
                _push(ssrRenderComponent(unref(link_default), {
                  ...
                  default: withCtx((_, _push2, _parent2, _scopeId) => {
                    if (_push2) {
                      _push2(`${ssrInterpolate(tarahi.user.name + "" + tarahi.user.lasst_name)}`);
                    } else {
                      return [
                        createTextVNode(toDisplayString(tarahi.user.name + "" + tarahi.user.lasst_name), 1)
                      ];
                    }
                  }),
                  _: 2
              ...
              _push(`<td>${ssrInterpolate(tarahi.plan.name)}</td><td>${ssrInterpolate(unref(moment)(tarahi.created_at).locale("fa", unref(fa)).format("jYYYY/jM/jD HH:mm"))}</td><td>${ssrInterpolate(unref(moment)(tarahi.updated_at).locale("fa", unref(fa)).format("jYYYY/jM/jD HH:mm"))}</td><td>`);
              if (tarahi.status == 0) {
              ...
              if (tarahi.status !== 3) {
                ...
                _push(ssrRenderComponent(unref(link_default), {
                  class: "dropdown-item",
                  href: _ctx.route("contractAdmin.show", [tarahi.id])
                }, {
                  default: withCtx((_, _push2, _parent2, _scopeId) => {
                    if (_push2) {
                      _push2(`نمایش`);
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Users/Admin/Contract/Contract-index.vue");
