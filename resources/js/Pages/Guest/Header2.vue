...
const form = useForm({ id:null,model:null,type:null,
...
const submit = ()=>{
    ...
    if (form.q) {
        form.get(route('website-templates.index')
        // ,{
        //     onFinish:() => submitTime()
        // }
        )
                ...
                <div class="row align-items-center">
                    <div class="col-xl-3 col-lg-4">
                        <div class="header-info">
                            <ul>
                                ...
                                <li><Link :href="route('about.index')">درباره ما</Link></li>
                            </ul>
                        </div>
                    </div>
                    ...
                    <div class="col-xl-3 col-lg-4">
                        <div class="header-info header-info-right">
                            <ul>
                                <li v-if="props.companies && props.companies.phone">کمک میخوایی ؟ شماره تماس : 
                                    <strong class="text-brand"> {{ '0' + props.companies.phone }}
                ...
                <div class="header-wrap">
                    ...
                    <div class="header-right"  >
                        <div class="search-style-2">
                            <form @submit.prevent="submit" v-if="$page.props.ziggy.location == 'http://localhost:8000/website-templates'">
                                <select class="select-active">
                                    <option>کل</option>
                                </select>
                                <input v-if="$page.props.ziggy.location == 'http://localhost:8000/website-templates' " type="search" v-model="form.q"  placeholder="اسم قالب مورد نظر خود را جستجو نمایید.">

                            </form>
                            <form @submit.prevent="submitTarahiFilter" v-if="$page.props.ziggy.location == 'http://localhost:8000/website-design'">
                                <select class="select-active">
                                    <option>کل</option>
                                </select>

                                <input v-if="$page.props.ziggy.location == 'http://localhost:8000/website-design' " type="search" v-model="form.q"  placeholder="عنوان پروژه مورد نظر خود را جستجو نمایید.">
                            </form>
										...
										<div class="cart-dropdown-wrap cart-dropdown-hm2" >
											<ul v-if="props.cart">
												<li v-for="(product,index) in props.cart.products" :key="index">
													...
													<div class="shopping-cart-delete">
														<a href="#" @click.prevent="submitRemove(index,product['model'])"><i class="fi-rs-cross-small"></i></a>
													</div>
												</li>
											...
											<div class="shopping-cart-footer" v-if="props.cart && props.cart.total > 0">
												<div class="shopping-cart-total" v-if="props.cart">
													<h4 class="d-flex">تعداد <span class="ms-auto">{{ (props.cart.count).toLocaleString("fa-IR") }}</span></h4>
												</div>
									...
									<div class="header-action-icon-2" v-if="$page.props.auth.user !== null">
										...
										<div class="cart-dropdown-wrap cart-dropdown-hm2 account-dropdown">
											<ul v-if="$page.props.auth.user !== null">
												...
												<li><a  @click.prevent="router.post(route('logout'))"><i class="fi fi-rs-sign-out mr-10"></i>خروج</a></li>
											</ul>
										</div>
									</div>
								</div>
                        ...
                        <div class="main-menu main-menu-padding-1 main-menu-lh-2 d-none d-lg-block font-heading">
                            <nav>
                                <ul>
                                        ...
                                        <li>
                                            <Link :href="route('website-templates.index','q')+'all' ">محصولات<i class="fi-rs-angle-down"></i></Link>
                                            <ul class="sub-menu" v-if="menus" v-for="(menu,index) in menus" :key="index">
                                                <li v-for="(men,index) in menu.children" :key="index" >
                                                        <Link :href="route('website-templates.index','type')+men.id+'#result'"  >{{menu.name +' '+ men.name}}</Link>
                                                </li>
                                            </ul>
                                        </li>
                                        <li>
                                            <Link :href="route('website-design.index','q')+'all' ">طراحی وبسایت<i class="fi-rs-angle-down"></i></Link>
                                            <ul class="sub-menu" v-if="menusTarahi" v-for="(menu,index) in menusTarahi" :key="index">
                                                <li v-for="(men,index) in menu.children" :key="index" >
                                                        <Link :href="route('website-design.index','type')+men.id+'#result'"  >{{menu.name +' '+ men.name}}</Link>
                                                </li>
                                            </ul>
                                        </li>
                                        <li>
                                            <Link :href="route('project.index','q')+'all' ">پروژه<i class="fi-rs-angle-down"></i></Link>
                                             <ul class="sub-menu" v-if="menusProject" v-for="(menu,index) in menusProject" :key="index">
                                                <li v-for="(men,index) in menu.children" :key="index" >
                                                        <Link :href="route('project.index','type')+men.id+'#result'"  >{{menu.name +' '+ men.name}}</Link>
                                                </li>
                                            </ul> 
                                        </li>
                                        <li>
                                            <Link :href="route('blog.index')">بلاگ<i class="fi-rs-angle-down"></i></Link>
                                            <ul class="sub-menu" v-if="menusBlog" >
                                                <li  v-for="(menu,index) in menusBlog" :key="index" >
                                                        <Link :href="route('blog.index','type')+menu.id+'#result'"  >{{menu.name}}</Link>
                                                </li>
                                            </ul>
                                        </li>
                                    <li class="position-static"><a href="#">مگا منو <i class="fi-rs-angle-down"></i></a>
                                        <ul class="mega-menu">
                                            <li class="sub-mega-menu sub-mega-menu-width-22">
                                                <Link class="menu-title" :href="route('website-templates.index')">محصولات</Link>
                                            ...
                                            <li class="sub-mega-menu sub-mega-menu-width-22">
                                                <Link class="menu-title" :href="route('website-design.index')">طراحی وسایت</Link>
                                            ...
                                            <li class="sub-mega-menu sub-mega-menu-width-22">
                                                <Link class="menu-title" :href="route('project.index')" >پروژه</Link>
                                                 <ul v-if="menusProject" v-for="(menu,index) in menusProject" :key="index">
                                                    <li v-for="(men,index) in menu.children" :key="index" >
                                                        <Link :href="route('project.index','type')+men.id+'#result'"  >{{menu.name +' '+ men.name}}</Link>
                                                    </li>
                                            ...
                                            <li class="sub-mega-menu sub-mega-menu-width-22">
                                                <Link class="menu-title" :href="route('blog.index')">بلاگ</Link>
                                                ...
                                                <div class="menu-banner-wrap">
                                                    ...
                                                    <div class="menu-banner-content">
                                                        ...
                                                        <div class="menu-banner-btn">
                                                            <Link :href="route('website-templates.index','sort')+'Discount'">هم حالا خرید کن </Link>
                                                        </div>
                                                    </div>
        ...
        <div class="mobile-header-wrapper-inner">
            ...
            <div class="mobile-header-content-area">
                ...
                <div class="mobile-menu-wrap mobile-header-border">
                    ...
                    <nav>
                        <ul class="mobile-menu font-heading">
                            ...
                            <li class="menu-item-has-children">
                                <Link :href="route('website-templates.index','q')+'all' " >محصولات</Link>
                            ...
                            <li class="menu-item-has-children">
                                <Link :href="route('website-design.index','q')+'all' ">طراحی وب سایت</Link>
                            ...
                            <li class="menu-item-has-children">
                                <Link :href="route('project.index','q')+'all' ">پروژه</Link>
                                 <ul class="dropdown"  v-if="menusProject" v-for="(menu,index) in menusProject" :key="index">
                                    <li v-for="(men,index) in menu.children" :key="index">
                                        <Link :href="route('project.index','type')+men.id+'#result'">{{menu.name +' '+ men.name}} </Link>
                                    </li>
                            ...
                            <li class="menu-item-has-children">
                                ...
                                <ul class="dropdown" v-if="menusBlog" >
                                    <li v-for="(menu,index) in menusBlog" :key="index" >
                            ...
                            <li class="menu-item-has-children">
                                ...
                                <ul class="dropdown">
                                    ...
                                    <li class="menu-item-has-children">
                                        ...
                                        <ul class="dropdown" v-if="menusBlog"  >
                                            <li>
                                                <template v-for="(menu,index) in menusBlog" :key="index">
                                                    <Link :href="route('blog.index','type')+menu.id+'#result'">{{menu.name +' '}}</Link>
                                                </template>
                ...
                <div class="mobile-header-info-wrap">
                    ...
                    <div class="single-mobile-header-info" v-else>
                        <Link :href="route('dashboard.index')"><i class="fi-rs-user"></i>داشبورت </Link>
                    </div>
                    <div class="single-mobile-header-info" v-if="props.companies && props.companies.phone" >
                        <Link href="#"><i class="fi-rs-headphones"></i>0{{ props.companies.phone }}
                            <template v-if="props.companies && props.companies.tel !== null"> -{{ '0' + props.companies.tel }}</template>
                        </Link>
                    </div>
                </div>
