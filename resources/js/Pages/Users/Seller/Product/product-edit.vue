...
const form =  useForm({id:props.product.id,name:props.product.name,name_en:props.product.name_en,
    ...
    test:[] , sub_test:{},tag:props.product.tag,slug:props.product.slug,keywords:props.product.keywords
});
                ...
                if(ruote.name == props.path)
                {
                    element.sections.forEach(section => {
                        ...
                        if(section.name == 'products' && props.product.group.id == element.id || 
                            section.name == 'forms' && props.product.group.id == element.id
                         )
                        {
                            ...
                            if (child.sections.length > 0)
                            {
                                child.sections.forEach(section =>
                                {
                                    if(section.name == 'products' && props.product.type.id == child.id ||
                                        section.name == 'forms' && props.product.type.id == child.id 
                                    )
                                    {
                            ...
                            if (child.sections.length > 0)
                            {
                                child.sections.forEach(section => {
                                    if(section.name == 'products' && props.product.category.id == child.id || 
                                        section.name == 'forms' && props.product.category.id == child.id
                                    )
                                    {
                            ...
                            if (element.sections.length > 0)
                            {
                                element.sections.forEach(section => {
                                    if (section.name == 'tests' && element.parent_id) {
                                        ...
                                        // گزینه‌های مولتی سلکت
                                        test_version.value.push(element);

                                        // مقدار انتخاب شده
                                        if (!form.sub_test[element.parent_id]) {
                                            form.sub_test[element.parent_id] = [];
                                        }

                                        form.sub_test[element.parent_id].push(element);
                                        }
                                    });
                            }
                        }
                    ...
                    <div class="col-lg-12">
                        <div class="card" v-if="step == 'step'">
                            ...
                            <div class="card-body">
                                ...
                                <div class="row">
                                    <div class="mt-1 col-sm-6">
                                        <label for="product_name" class="form-label">اسلاگ<span
                                                class="text-danger">*</span></label>
                                        <input v-model.lazy="form.slug" type="text" placeholder="اینجا تایپ کنید"
                                            class="form-control" id="product_name" />
                                    </div>
                                    <div class="mt-1 col-sm-6">
                                        <label for="product_name" class="form-label">کلمات کلیدی</label>
                                        <input v-model.lazy="form.keywords" type="text" placeholder="اینجا تایپ کنید"
                                            class="form-control" id="product_name" />
                                    </div>
                                </div>
                                <div class="mt-4">
                                ...
                                <div class="card-body">
                                     <div class="row gx-2">
                                        ...
                                        <div class="col-lg-6" v-if="form.prerequisites" v-for="prerequisite,index in form.prerequisites" :key="index" >
                                            ...
                                            <VueMultiselect
                                                v-model="form.prerequisite_version[prerequisite.id]"
                                                :multiple="true"
                                                :options="prerequisite_version.filter(
                                    ...
                                    <div class="row gx-2">
                                     ...
                                     <div class="row gx-2">
                                        ...
                                        <div class="col-lg-6" v-if="form.test" v-for="tes,index in form.test" :key="index" >
                                            <label class="form-label">نسخه تست شده محصول ({{tes.name}})</label>
                                            <VueMultiselect
                                                v-model="form.sub_test[tes.id]"
                                                :multiple="true"
                                                :options="test_version.filter(
                                    ...
                                    <div class="row gx-2">
                                        <label class="form-label">تگ توضیحات سئو: طول کارکتر بین ۱۲۰ تا ۱۶۰ کاراکتر و شامل کلمه کلیدی اصلی</label>
                                        <textarea v-model="form.tag" class="form-control w-100" name="comment" id="comment" cols="30" rows="9" placeholder="اینجا تایپ کنید"></textarea>
                                    </div>
                                </div>
                        </div>
                    </div>
